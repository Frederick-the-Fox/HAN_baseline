...
def load_data_dblp(path='/home/hangni/WangYC/HAN/data/mydata/freebase/freebase.mat'):
    data = sio.loadmat(path)
    truelabels, truefeatures = data['label'], data['feature'].astype(float)
    N = truefeatures.shape[0]
    rownetworks = [data['MAM'] - np.eye(N), data['MDM'] - np.eye(N) , data['MWM'] - np.eye(N)]
    y = truelabels
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
        ...
        print('final_embedding:{}, shape:{}'.format(jhy_final_embedding, jhy_final_embedding.shape))
        np.save(dataset + patience + '.npy', jhy_final_embedding)
        print('saved')
