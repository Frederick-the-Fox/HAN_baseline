...
def load_data_dblp(path='/home/hangni/WangYC/HAN/data/mydata/dblp/dblp.mat'):
    data = sio.loadmat(path)
    truelabels, truefeatures = data['label'], data['feature'].astype(float)
    N = truefeatures.shape[0]
    rownetworks = [data['APA'] - np.eye(N), data['APCPA'] - np.eye(N) , data['APTPA'] - np.eye(N)]
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
        ...
        np.save(dataset+patience, jhy_final_embedding)
        print('saved')
