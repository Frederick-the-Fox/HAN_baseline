# This is only Baseline code!
> by WangYC
> @NWPU changan Apr.18th 2022

## baseline使用方法：
### 训练
环境要求：tensorflow-gpu=1.15.0 cuda=10.0 python=3.6
首先找到对应的ex_dataset.py文件
1. 修改要用来训练的gpu编号
2. python ex_dataset.py argv[1]
其中的argv是patience

### 测试
环境要求：pytorch=1.7.0 python=3.8
evaluate.py
修改：
1. 数据集名称
2. metapath集合
3. 分类数量
python evaluate.py argv[1]
其中的argv还是patience
